...
class NewBeamtimeTest(unittest.TestCase): 
    ...
    def setUp(self):
        ...
        self.backup_dir= os.path.join(remote_dir, strftime('%Y'), 'userBeamtimeArchive')
    ...
    def test_check_empty_environment(self):
        #sanity check. xpdUser directory exists.  First make sure it does exist.
        self.assertTrue(os.path.exists(self.home_dir))
        self.assertRaises(RuntimeError, lambda:os.path.isdir(self.home_dir))
        
        #sanity check. xpdUser directory exists.  First make sure it doesn't exist.
        self.assertTrue(os.path.exists(self.backup_dir))
        self.assertRaises(RuntimeError, lambda:os.path.isdir(self.backup_dir))

        
        #now put things could appear during beamtime in beamtime
