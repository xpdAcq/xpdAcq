...
from xpdacq.beamtimeSetup import _make_clean_env, _start_beamtime, \
    _end_beamtime, _execute_start_beamtime, _check_empty_environment
import xpdacq.beamtimeSetup as bts
from xpdacq.beamtime import XPD, Beamtime
from xpdacq.glbl import glbl
from xpdacq.beamtime import _clean_name, _clean_md_input, _update_objlist, \
    _get_yaml_list, _get_hidden_list
...
class NewExptTest(unittest.TestCase):
    ...
    def test_Scan_validator(self):
        ...
        self.assertRaises(TypeError, lambda: Scan(1,
                                                  'ct10s'))  # give Beamtime but not Sample
    ...
    def test_auto_naming_ScanPlan(self):
        ...
        '''
        self.assertRaises(SystemExit, lambda: ScanPlan(
            'ct_5_25575_32767'))  # extra argument
        self.assertRaises(SystemExit, lambda: ScanPlan(
            'Tramp_5_300_200'))  # incomplete arguments
        self.assertRaises(SystemExit, lambda: ScanPlan(
            'Tramp_5_300_200_5_1111'))  # extra argument
        self.assertRaises(SystemExit, lambda: ScanPlan(
            'tseries_5_60'))  # incomplete arguments
        self.assertRaises(SystemExit, lambda: ScanPlan(
            'tseries_5_60_10_1111'))  # extra argument
        # additional unit
