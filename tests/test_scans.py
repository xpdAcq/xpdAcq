...
from xpdacq.xpdacq import prun, calibration, dark, dryrun, background, \
    _auto_dark_collection, _auto_load_calibration_file
...
class NewScanTest(unittest.TestCase):
    def setUp(self):
        ...
        self.stbt_list = ['bt_bt.yml', 'ex_l-user.yml', 'sa_l-user.yml',
                          'sp_ct.1s.yml', 'sp_ct.5s.yml', 'sp_ct1s.yml',
                          'sp_ct5s.yml', 'sp_ct10s.yml', 'sp_ct30s.yml']
    ...
    def test_auto_dark_collection(self):
        ...
        self.assertTrue(
            'sc_dk_field_uid' in auto_dark_md_dict_set_dk_window and 'sc_dk_field_uid' in auto_dark_md_dict)
    ...
    def test_auto_load_calibration(self):
        ...
        self.assertEqual(auto_calibration_md_dict['sc_calibration_parameters'][
                             'Experiment']['integrationspace'], 'qspace')
        self.assertEqual(
    ...
    def test_new_prun_with_auto_dark_and_auto_calibration(self):
        self.sp = ScanPlan('ct', {'exposure': 0.1, 'dk_window': 32767},
                           dk_window=32767, shutter=False)
        self.sc = Scan(self.sa, self.sp)
        ...
        self.assertEqual(glbl.xpdRE.call_args_list[-1][1]['sp_dk_window'],
                         32767)
        # is calibration loaded?
        self.assertEqual(
            glbl.xpdRE.call_args_list[-1][1]['sc_calibration_file_name'],
            cfg_f_name)
