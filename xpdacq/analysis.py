...
def _feature_gen(header):
    ...
    time_stub = _timestampstr(header.start.time, hour=True)
    feature_list = []
...
def _timestampstr(timestamp, hour=False):
    ...
    if not hour:
        timestring = datetime.datetime.fromtimestamp(float(timestamp)).strftime('%Y_%m_%d')
    elif hour:
        timestring = datetime.datetime.fromtimestamp(float(timestamp)).strftime('%Y_%m_%d-%H_%M')
    return timestring
