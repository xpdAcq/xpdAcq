...
def dark_light():
    ...
    dark = yield from bps.trigger_and_read(xpd_configuration['area_det'],
                                           name='dark')
    # take data
...
def tune_filters(upper_threshold=8000,
    ...
    def inner():
        ...
        for fc in filter_configurations:
            ...
            yield from (bps.abs_set(f, io, wait=True) for f, io in
                        zip(xpd_configuration['filter_bank'], fc))
        ...
        fci = np.where((sp * attenuations * desired_exposure
                        ) < upper_threshold)[0][-1]
        # set the bank
        yield from (bps.abs_set(f, io, wait=True) for f, io in
                    zip(xpd_configuration['filter_bank'],
