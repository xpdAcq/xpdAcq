...
def import_userScriptsEtc():
    ...
    if failure_list:
        print(
            "Finished importing. Failed to move {}"
...
class ExceltoYaml:
    ...
    _NAME_FIELD = list(map(lambda x: x.lower().replace(" ", "_"), NAME_FIELD))
    _COMMA_SEP_FIELD = list(
        map(lambda x: x.lower().replace(" ", "_"), COMMA_SEP_FIELD)
    )
    _SAMPLE_NAME_FIELD = list(
        map(lambda x: x.lower().replace(" ", "_"), SAMPLE_NAME_FIELD)
    )
    _BKGD_SAMPLE_NAME_FIELD = list(
        map(lambda x: x.lower().replace(" ", "_"), BKGD_SAMPLE_NAME_FIELD)
    )
    _PHASE_FIELD = list(
        map(lambda x: x.lower().replace(" ", "_"), PHASE_FIELD)
    )
    _DICT_LIKE_FIELD = list(
        map(lambda x: x.lower().replace(" ", "_"), DICT_LIKE_FIELD)
    )
    ...
    def create_yaml(self, bt):
        ...
        """
        sample_name_set = set(
            [d["sample_name"] for d in self.parsed_sa_md_list]
        )
        ...
        if no_bkgd_sample_name_list:
            print(
                "INFO: If you want to associate a background sample,"
                ...
                "associated background samples):\n{}\n".format(
                    self._BKGD_SAMPLE_NAME_FIELD, no_bkgd_sample_name_list
                )
            )
    ...
    def parse_phase_info(cls, phase_str):
        ...
        parsed_phase_md = {}
        try:
            (composition_dict, phase_dict, composition_str) = cls.phase_parser(
                phase_str
            )
        except ValueError:
...
def import_sample_info(saf_num=None, bt=None, validate_only=False):
    ...
    _import_sample_info(saf_num=saf_num, bt=bt, validate_only=validate_only)
...
def _import_sample_info(saf_num=None, bt=None, validate_only=False):
    ...
    if not isinstance(bt, Beamtime):
        raise TypeError(
            "WARNING: illegal Beamtime object argument!.\n"
            "input object {} has type = {}\n"
            "Please make sure you are handing correct object\n"
            "or double check if you already started a beamtime".format(
                bt, type(bt)
            )
        )
        return
    ...
    if saf_num is None:
        try:
            ...
            print(
                "WARNING: there is no SAF number information in this "
        ...
        if saf_num != saf_num_from_bt:
            raise ValueError(
                "WARNING: you give a SAF number = {}, "
                "while SAF number of current beamtime is = {}\n"
                "Please make sure you are using the correct "
                "SAF number/beamtime combination".format(
                    saf_num, saf_num_from_bt
                )
            )
            return
    ...
    if validate_only:
        ...
        print(
            "INFO: please set 'validate_only=False' and "
